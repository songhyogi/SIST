...
public interface TalkMapper {
	...
	public Integer selectRowCount(Map<String,Object> map);//채팅방이 여러 개일 수 있으니까 목록으로 처리하기 위해
	//채팅방 번호 생성
	...
	public void insertTalkRead(@Param(value="talkroom_num")Long talkroom_num,@Param(value="talk_num")Long talk_num,@Param(value="mem_num")Long mem_num);//마이바티스는 맵이나 자바빈에 넣는다던가 해서 하나의 파라미터를 사용해야 하는데 여러 개의 파라미터를 사용하고 싶으면 @Param을 사용해야 한다.
	//채팅 메시지 읽기
	public List<TalkVO> selectTalkDetail(Map<String,Long> map);
	//읽은 채팅 기록 삭제
	@Delete("DELETE FROM sptalk_read WHERE talkroom_num=#{talkroom_num} AND mem_num=#{mem_num}")//맵형태로 넘어오기 때문에 talkroom_num AND mem_num으로 명시한 것이다.
