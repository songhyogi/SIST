...
<div class="page-main">
	...
	<form action="list" id="search_form" method="get">
		<input type="hidden" name="category" value="${param.category}"><!-- 카테고리가 없으면 전체로 -->
		<ul class="search">
			<li>
		...
		<div class="align-right">
			...
			<script type="text/javascript">
				$('#order').change(function(){<%--카테고리 선택하게 되면 계속 세팅되게--%>
					location.href='list?category=${param.category}&keyfield='
										+$('#keyfield').val()+'&keyword='
										+$('#keyword').val()+'&order='
										+$('#order').val();
					<%--category는 파라미터로 넘어온 값. keyfield,keyword,order는 select태그에서 세팅되어 있는 값--%>
				});
			</script>
