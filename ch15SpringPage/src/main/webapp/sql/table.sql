...
create table spmember(
 ...
 nick_name varchar2(30),--처음에는 닉네임 입력하지 않아도 되게 null 허용
...
create table spmember_detail(
 mem_num number not null,
 au_id varchar2(36) unique, --자동로그인에 사용되는 식별값. unique하지 않으면 에러가 나기 때문에 unique하지 않으면 처리가 되지 않게 하기 위해 unique로 함
 ...
 photo blob, --table에 바이너리 데이터를 blob형태로 넣을 수 있다. 프로필 사진만 테이블에 저장. 테이블에 넣으면 느려진다.
 photo_name varchar2(100), --파일명은 생략 가능하다. (blob로 들어갔을 때 파일명은 생략 가능)
...
create table spboard_response(
 ...
 te_mdate date,
 te_parent_num number not null, --부모글의 번호가 들어감, 자식글이 아니라 부모글일 경우 0 (0이 들어가기 떄문에 foreign key제약 조건을 주지 못한다)
