		...
		$.ajax({
			...
			success:function(param){
				...
				if(currentPage>=Math.ceil(count/rowCount)){
					//다음 페이지가 없음
					$('.paging-button').hide();//다음 페이지가 없을 경우 버튼 숨김
				}else{
					//다음 페이지가 존재
					$('.paging-button').show();//다음 페이지가 존재할 경우 버튼 보임
				}
			},
			error:function(){
				alert('네트워크 오류');
			...
			$.ajax({
				...
				success:function(param){
					if(param.result == 'logout'){
						...
						$('#mresp_form').parent().find('p').html($('#mresp_content').val()
												.replace(/</g,'&lt;').replace(/>/g,'&gt;')
												.replace(/\r\n/g,'<br>').replace(/\r/g,'<br>').replace(/\n/g,'<br>'));
					//최근 수정일 처리
		...
		$.ajax({
			...
			success:function(param){
				if(param.result == 'logout'){
					...
					if(param.cnt>0){
						getListResponse(re_num,resdelete_btn.parents('.sub-item'));//0보다 크면 삭제된 거 빼고 읽어와야 한다.
					}else{
