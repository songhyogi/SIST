...
public class MemberDAO {
	...
	//회원상세정보 
	public MemberVO getMember(int num)throws Exception{//void는 콘솔에서 출력했을때 지금은 jsp에서 출력으로 데이터 반환-> 반환한 거에 데이터 꺼내서 jsp에 표시
		Connection conn = null;
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		...
		try {
			...
			if(rs.next()) {//서버프로그램의 데이터이기 때문에 필요한 것만 뽑아서 사용할 수 있다.
				member = new MemberVO();
