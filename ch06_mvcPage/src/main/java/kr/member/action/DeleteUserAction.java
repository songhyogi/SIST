...
public class DeleteUserAction implements Action{
	...
	public String execute(HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		//로그인한 아이디
		String user_id = (String)session.getAttribute("user_id");//로그인한 아이디로 탈퇴할 수 있도록 로그인한 아이디를 받는다.
		...
		if(check) {//인증 성공
			...
			//프로필 사진 삭제
			FileUtil.removeFile(request, db_member.getPhoto());//세션에서 빼도 되고 db에서 빼도 되고
			//로그아웃
			session.invalidate();
		}
