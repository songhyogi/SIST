...
public class UserModifyAction implements Action{
	...
	public String execute(HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		//주문정보 호출
		OrderVO db_order = dao.getOrder(order_num);//한건의 레코드를 구함
		if(db_order.getMem_num() != user_num) {//다른 사람의 정보를 바꾸는 것을 막기 위함
			//구매자 회원번호와 로그인한 회원번호가 불일치할 경우
			return "/WEB-INF/views/common/notice.jsp";
		}
		
		//배송지 수정 전 배송상태를 한 번 더 체크
		//오동작을 막기 위해서 변경하기 직전에 다시 한 번 더 체크
