...
public class AdminModifyStatusAction implements Action{
	...
	public String execute(HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		//주문정보 반환
		OrderVO db_order = dao.getOrder(order.getOrder_num());//최종적으로 status를 확인하기 위해서(간발의 차이로 정보를 변경할 수 있으므로)
		
		//사용자가 배송상태를 5로 변경했을 경우
		if(db_order.getStatus()==5) {
			request.setAttribute("notice_msg", "사용자가 배송상태를 주문 취소로 변경해서 관리자가 배송상태 수정 불가능");
