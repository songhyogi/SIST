...
public class OrderDAO {
	...
	public void insertOrder(OrderVO order, List<OrderDetailVO> orderDetailList)throws Exception{
		...
		try {
			...
			//order_num 구하기 zorder과 zorder_detail에 동일한 order_num을 넣으려면 시퀀스를 동일하게 생성해서 공유해야 한다.
			...
			//주문정보 처리
			sql = "INSERT INTO zorder (order_num,order_total,payment,"
					+ "receive_name,receive_post,receive_address1,receive_address2,"
					+ "receive_phone,notice,mem_num) VALUES (?,?,?,?,?,?,?,?,?,?)";
			...
			for(int i=0;i<orderDetailList.size();i++) {
				...
				if(i % 1000 == 0) {//1000개가 되면 전송
					pstmt3.executeBatch();
				}
			}
			pstmt3.executeBatch();//쿼리를 전송 (1000개가 안 되거나 1000개 보내고 남은 것도 전송) 
	...
	public List<OrderVO> getListOrder(int start, int end, String keyfield, String keyword)throws Exception{
		...
		try {
			...
			while(rs.next()) {
				...
				order.setId(rs.getString("id"));//조인해서 읽어옴
	...
	public int getOrderCountByMem_num(String keyfield,String keyword,int mem_num)throws Exception{
		...
		try {
			...
			sql = "SELECT COUNT(*) FROM zorder JOIN (SELECT order_num, "
					+ "LISTAGG(item_name,',') WITHIN GROUP (ORDER BY item_name) "
					+ "item_name FROM zorder_detail GROUP BY order_num) "
					+ "USING (order_num) WHERE mem_num=? " + sub_sql;//WHERE 뒤에 sub_sql이 오기 때문에 AND로 연결되어야 한다.
			//PreparedStatement 객체 생성
			pstmt = conn.prepareStatement(sql);
			//?에 데이터 바인딩
		...
		try {
			...
			if(order.getStatus() == 5) {
				...
				for(int i=0;i<detailList.size();i++) {
					...
					pstmt2.addBatch();//반복문을 이용할 때는 배치를 사용
	...
	public void updateOrderCancel(int order_num)throws Exception{
		...
		try {
			...
			conn.setAutoCommit(false);
			//SQL문 작성
			sql = "UPDATE zorder SET status=5,modify_date=SYSDATE WHERE order_num=?";//취소는 무조건 5이기 때문에 status값을 전달받지 않아도 됨 바로 status=5라고 넣음
			pstmt = conn.prepareStatement(sql);
			pstmt.setInt(1, order_num);
			pstmt.executeUpdate();
			
			//주문번호에 해당하는 상품정보 구하기 (이미 주문번호에 해당하는 상품정보를 구하는 메서드가 만들어져 있으면 그 메서드를 호출하거나 sql문을 하나 더 만들어서 구해도 됨)
			List<OrderDetailVO> detailList = getListOrderDetail(order_num);//이미 만들어져있는 메서드가 있으므로 메서드를 활용 (코드 절약)
			//주문 취소로 주문상품의 재고 수 환원
			sql = "UPDATE zitem SET quantity=quantity + ? WHERE item_num=?";//종류가 다른 여러개의 상품이 있을 수 있기 때문에 루프를 돌면서 상품 하나당 이 작업을 실행
			pstmt2 = conn.prepareStatement(sql);
			for(int i=0;i<detailList.size();i++) {
				OrderDetailVO detail = detailList.get(i);
				pstmt2.setInt(1, detail.getOrder_quantity());
				pstmt2.setInt(2, detail.getItem_num());
				pstmt2.addBatch();//양이 적더라도 기본적으로 배치형태로 만들어놓는 것이 좋음
