...
public class CartDAO {
	...
	public int getTotalByMem_num(int mem_num)throws Exception{
		...
		try {
			...
			sql = "SELECT SUM(sub_total) FROM "
					+ "(SELECT mem_num,order_quantity * price AS sub_total FROM zcart JOIN zitem USING(item_num)) "
					+ "WHERE mem_num=?";
	...
	public List<CartVO> getListCart(int mem_num)throws Exception{
		...
		try {
			...
			conn = DBUtil.getConnection();
			//SQL문 작성
			sql = "SELECT * FROM zcart c JOIN zitem i USING(item_num) WHERE mem_num=? ORDER BY c.reg_date DESC";//두 군데에 reg_date가 있기 때문에 알리아스 추가
			//PreparedStatement 객체 생성
			pstmt = conn.prepareStatement(sql);
			//?에 데이터 바인딩
			pstmt.setInt(1, mem_num);
			...
			while(rs.next()) {
				...
				//동일 상품(item_num이 같은 상품)의 총 구매 금액 구하기
				cart.setSub_total(cart.getOrder_quantity()*item.getPrice());//가격이 변동할 수 있기 때문에 sub_total을 데이터베이스에 저장하지 않고 자바빈에만 저장함
	...
	public CartVO getCart(CartVO cart)throws Exception{
		...
		try {
			...
			conn = DBUtil.getConnection();
			//SQL문 작성
			sql = "SELECT * FROM zcart WHERE item_num=? AND mem_num=?";//item_num과 mem_num을 보내서 존재하는지 확인
			//PreparedStatement 객체 생성
			pstmt = conn.prepareStatement(sql);
			//?에 데이터 바인딩
			pstmt.setInt(1, cart.getItem_num());
			...
			//SQL문 실행
			rs = pstmt.executeQuery();
			if(rs.next()) {//item_num과 mem_num이 유니크하기 때문에 한건의 데이터
