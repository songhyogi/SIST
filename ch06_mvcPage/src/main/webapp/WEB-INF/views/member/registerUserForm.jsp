...
<script type="text/javascript" src="${pageContext.request.contextPath}/js/jquery-3.7.1.min.js"></script><%-- jquery를 사용할거니까 --%>
<script type="text/javascript">
$(function(){//$(document).ready(function(){})의 축약형
	let idChecked = 0; //0:중복,아이디 중복 미실시 1:미중복
	
	//아이디 중복 체크
	$('#id_check').click(function(){
		if(!/^[A-Za-z0-9]{4,12}$/.test($('#id').val())){//영문, 숫자 최소 4자~최대12자까지 입력하도록 정규표현식으로 조건체크
			alert('영문 또는 숫자 사용, 최소 4자 ~ 최대 12자 사용');
			$('#id').val('').focus();
			return;
		}
		//위에 있는 if문에 진입하면 return, 위에 있는 if문에 진입을 하지 않은거면 정상적으로 입력한 거니까 서버와 통신
		//서버와 통신
		$.ajax({
	...
	$('#register_form #id').keydown(function(){
		idChecked = 0; //키이벤트가 발생하면 아이디 중복체크 미실시한 걸로
		$('#message_id').text(''); //키이벤트가 발생할 때마다 초기화
	});//end of key down
	...
	<div class="content-main">
		...
		<form id="register_form" action="registerUser.do" method="post">
			<ul>
				...
				<li>
					<label for="zipcode">우편번호</label>							<!-- 다음에서 다음 우편번호 api를 사용할 거라서 autocomplete는 off로 -->
					<input type="text" name="zipcode" id="zipcode" maxlength="5" autocomplete="off" class="input-check">
					<input type="button" value="우편번호 찾기" onclick="execDaumPostcode()"><!-- 아래 다음 우편번호 API 부분을 보면 execDaumPostcode()가 있다 -->
				</li>
				<li>
			...
			<div class="align-center">
				...
				<input type="button" value="홈으로" onclick="location.href='${pageContext.request.contextPath}/main/main.do'"><!-- 다른 경로이기 때문에 uri방식으로 명시 -->
			</div>
		</form>
		<!-- 다음 우편번호 API 시작 -->
