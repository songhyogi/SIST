		...
		<h2>구매상세</h2>
			...
			<ul id="delivery_info">
				...
				<li class="align-center">
					...
					<input type="button" value="주문취소" id="order_cancel"><%-- id를 만들었다는 것은 스크립트를 만들겠다는 의도니까 input안에 onclick으로 주소를 따로 또 명시해주면 안된다. --%>
					<script>
						const order_cancel = document.getElementById('order_cancel');
						order_cancel.onclick=function(){
							let choice = confirm('주문을 취소하시겠습니까?');
							if(choice){
								location.replace('orderCancel.do?order_num=${order.order_num}');<%-- 삭제할때는 history가 남아있을 필요가 없으니까 replace로 history 삭제 --%>
							}
						};
					</script>
					</c:if>
