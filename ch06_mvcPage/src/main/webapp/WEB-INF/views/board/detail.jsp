...
<div class="page-main">
	...
	<div class="content-main">
		...
		<!-- 파일을 업로드 할 수도 있고 업로드하지 않을 수 있기 때문에 조건체크가 필요함 -->
		<c:if test="${!empty board.filename}"><!-- 파일명이 있을 경우 -->
		...
		<ul class="detail-sub">
			...
			<li>
				<c:if test="${!empty board.modify_date}"><!-- 수정한 적이 있으면 수정일이 보여짐 -->
				...
				<c:if test="${user_num == board.mem_num}">
					...
					<script type="text/javascript">
						...
						delete_btn.onclick=function(){
							...
							if(choice){//choice가 true라는 건 확인버튼을 눌렀다는 의미
		...
		<div id="reply_div">
			...
			<form id="re_form">
				...
				<c:if test="${empty user_num}">disabled="disabled"</c:if><%-- 로그인이 안 되어 있을 경우 비활성화 --%>
					id="re_content" 
				class="rep-content"><c:if test="${empty user_num}">로그인해야 작성할 수 있습니다.</c:if></textarea>
				<c:if test="${!empty user_num}"><%--로그인이 되어 있을 경우 보여지게 --%>
				<div id="re_first">
