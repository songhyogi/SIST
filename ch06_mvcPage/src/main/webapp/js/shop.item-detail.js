$(function(){
	...
	$('#order_quantity').on('keyup mouseup',function(){
		if($('#order_quantity').val()==''){//비어있으면 주문수량이 없는 거니까 총 주문금액 0원이라고 보이게
			$('#item_total_txt').text('총 주문 금액 : 0원');
			return;
		}
		if($('#order_quantity').val()<=0){//0이거나 음수이면 비어있게(0과 음수를 불인정)
			$('#order_quantity').val('');
			$('#item_total_txt').text('총 주문 금액 : 0원');
			return;
		}
		...
		$('#item_total_txt').text('총 주문 금액 : ' + total.toLocaleString() + '원');//toLocaleString:숫자 세자리씩 끊어서
	});
